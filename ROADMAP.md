...
- [ ] Backend scaffold (Flask, requirements.txt, Makefile)
...
- [ ] `app.py`: /health, /markets, /markets/{id}/analyze
...
- [ ] `app.py`: /markets/{id} with full history, /analyses paginated
...
- [ ] `app.py`: PUT /markets/{id}/resolution
...
- [ ] ~~`scripts/import_calibration.py`: import from existing tracker~~ (not built — the calibration dataset was generated in-tool, not imported)
